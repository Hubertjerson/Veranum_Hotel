		...
		if (isset($_POST["registroNombre"])) {
			...
			if (
				...
				if ($respuesta == "ok") {
					...
					$mail->setFrom('jerson.huaman.11@outlook.com', 'Probando Correo');

					$mail->addReplyTo('jerson.huaman.11@outlook.com', 'Probando Correo');
		...
		if (isset($_POST["idUsuarioFoto"])) {
			...
			if (isset($_FILES["cambiarImagen"]["tmp_name"]) && !empty($_FILES["cambiarImagen"]["tmp_name"])) {
				...
				if ($_FILES["cambiarImagen"]["type"] == "image/jpeg" || $_FILES["cambiarImagen"]["type"] == "image/png") {
					...
					if (move_uploaded_file($_FILES["cambiarImagen"]["tmp_name"], $ruta)) {
						// Eliminar la imagen anterior si existe
						if ($_POST["fotoActual"] && $_POST["fotoActual"] != "") {
							unlink("backend/" . $_POST["fotoActual"]);
						}
						...
						$item = "foto";
						$valor = substr($ruta, 8); // Remover 'backend/' de la ruta
						...
						if ($actualizarFotoPerfil == "ok") {
                            ...
                            Swal.fire({
                                type: "error",
                                title: "¡ERROR!",
                                text: "¡Error al actualizar la foto de perfil en la base de datos!",
                                showConfirmButton: true,
                                confirmButtonText: "Cerrar"
                            }).then(function(result){
                                if(result.value){
			...
			if (preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["emailRecuperarPassword"])) {
				...
				if ($traerUsuario) {
					...
					if ($actualizarPassword  == "ok") {
						...
						$mail->setFrom('jerson.huaman.11@outlook.com', 'Probando veficacion');

						$mail->addReplyTo('jerson.huaman.11@outlook.com', 'Probando veficacion');
